...
## Config Network
max_hd = 700 # maximal number of hidden units
min_hd = 20 # minimal number of hidden units
hds = np.linspace(min_hd,max_hd,max_hd/min_hd,endpoint=True)
...
training_acc_before = np.zeros(int(max_hd/min_hd))
training_acc_after = np.zeros(int(max_hd/min_hd))
test_acc_before = np.zeros(int(max_hd/min_hd))
test_acc_after = np.zeros(int(max_hd/min_hd))
num_of_operation = np.zeros(int(max_hd/min_hd))
...
for iteration in range(num_of_it):
	print(iteration)
	ind = 0
	for hidden_neurons in hds.astype(int):
		...
		# Train agian after Pruning
		Y_pred,_ = net(X)
		_, predicted = torch.max(Y_pred, 1)
		total = predicted.size(0)
		correct = predicted.data.numpy() == Y.data.numpy()
		training_acc_after[ind]+=(100 * sum(correct)/total)
		# Test agian after Pruning
		Y_pred_test,_ = net(X_test)
		_, predicted_test = torch.max(Y_pred_test, 1)
		# calculate accuracy
		total_test = predicted_test.size(0)
...
ax1.plot(hds,training_acc_before,'ro-')
ax1.plot(hds,training_acc_after,'bo-')
ax1.plot(hds,test_acc_before,'ro-')
ax1.plot(hds,test_acc_after,'bo-')
...
ax1.axis([0,max_hd,0,100])

ax2 = fig1.add_subplot(122)
ax2.plot(hds,num_of_operation,'ro-')
ax2.plot(hds,np.absolute(np.array(training_acc_before)-np.array(training_acc_after)),'bo-')
plt.legend(['Reduction rate','Difference between training accuracies'],loc='best')
ax2.set_xlabel('Number of Hidden Units')
ax2.xaxis.set_major_locator(MaxNLocator(integer=True))
#ax2.axis([0,max_hd,0,10])
plt.show()
