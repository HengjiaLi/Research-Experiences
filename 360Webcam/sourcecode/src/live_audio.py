...
class MicArray(object):
    ...
    def __init__(self, rate=16000, channels=8, chunk_size=None):
        ...
        self.player=self.pyaudio_instance.open(format=pyaudio.paInt16,
		rate=int(self.sample_rate),channels=self.channels,Output=true,
		frames_per_buffer=int(self.chunk_size)
        )
        self.stream_play.write(x.tobytes())
		while True:
		stream audio
		data=np.fromstring(self.stream.read(self.chunk_size,exception_on_overflow = False),dtype=np.int16)
			player.write(data,self.chunk_size)
