...
class data_receiver(object):
    ...
    def __init__(self):
        ...
        self.cam_capture.set(cv.CAP_PROP_FOURCC,cv.VideoWriter_fourcc('M','J','P','G'))
        self.ret=None
        self.frame=None
    ...
    def panoroma_server(self):
        # open a camera file
#        dev = os.open("/dev/video6", os.O_WRONLY)
        dev = open("/dev/video6", 'wb')
        while self.cam_capture.isOpened():
            # read image
            self.get_frame()
            # undistort
            self.dewarp()
            # send dewapring frame to output
            if self.ret:
                ret,jpg=cv.imencode('.jpg',self.dewarping_frame)
                print(type(jpg.tobytes()))
#                os.write(dev,jpg.tobytes())
                dev.write(jpg.tobytes())
